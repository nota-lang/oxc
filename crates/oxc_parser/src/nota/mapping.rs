...
//!
//! The Nota reader lowers `@`-markup to a plain oxc `Program`. Embedded JS — prop values,
//! `@(expr)`/`@name` interpolation, `%`/`%%%` statement bodies, math interpolation, `@if`/`@for`
//! heads — is spliced as real oxc nodes carrying their *source* spans; component tags
//! (`@Aside` → `h(Aside, …)`) become real identifier references, also source-spanned.
...
//!
//! This module *exposes* that existing data as Volar `CodeMapping`s. The reader records a flat list
//! of [`NotaMappingMark`]s — `(source span, kind)` — at each embedded-JS splice / component tag. The
//! compile entry pairs each mark's `span.start` with the **generated** offset codegen emitted the
