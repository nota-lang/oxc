...
impl<'a, C: Config> ParserImpl<'a, C> {
    ...
    fn parse_primary_expression(&mut self) -> Expression<'a> {
        ...
        match kind {
            ...
            // `@` is overloaded: a Nota markup sigil in Nota source (`SourceType::is_nota`),
            // otherwise a JS/TS decorator. In expression position the form is an embedded-JS one
            // (a `Script` body frame), so its trailing context resumes as JS. The parser leaves the
            // form un-lowered (`Expression::NotaMarkup`); the separate lowering pass replaces it.
            Kind::At if self.source_type.is_nota() => self.parse_nota_markup_expression(),
