...
impl<'a> AstBuilder<'a> {
    ...
    /// Build a [`NotaChild::Link`].
    ///
    /// This node contains a [`NotaLink`] that will be stored in the memory arena.
    ///
    /// ## Parameters
    /// * `span`: The [`Span`] covering this node
    /// * `url`: The raw url slice (inside `(…)`, untrimmed/uncooked).
    /// * `url_span`: Source span of `url` — for the highlight pass.
    /// * `children`: The link text (inside `[…]`) — a bounded markup body.
    #[inline]
    pub fn nota_child_link<A1>(
        self,
        span: Span,
        url: A1,
        url_span: Span,
    ...
    where
        A1: Into<Str<'a>>,
    {
        NotaChild::Link(self.alloc_nota_link(span, url, url_span, children))
    }

    /// Build a [`NotaChild::Image`].
    ///
    /// This node contains a [`NotaImage`] that will be stored in the memory arena.
    ///
    /// ## Parameters
    /// * `span`: The [`Span`] covering this node
    /// * `alt`: The raw alt-text slice (inside `[…]`, uncooked).
    /// * `alt_span`: Source span of `alt` — for the highlight pass.
    /// * `src`: The raw src slice (inside `(…)`, untrimmed/uncooked).
    /// * `src_span`: Source span of `src` — for the highlight pass.
    #[inline]
    pub fn nota_child_image<A1, A2>(
        self,
        span: Span,
        alt: A1,
        alt_span: Span,
    ...
    ///
    /// If you want the built node to be allocated in the memory arena,
    /// use [`AstBuilder::alloc_nota_link`] instead.
    ///
    /// ## Parameters
    /// * `span`: The [`Span`] covering this node
    /// * `url`: The raw url slice (inside `(…)`, untrimmed/uncooked).
    ...
    where
        A1: Into<Str<'a>>,
    {
        NotaLink { node_id: Default::default(), span, url: url.into(), url_span, children }
    }

    /// Build a [`NotaLink`], and store it in the memory arena.
    ///
    /// Returns a [`Box`] containing the newly-allocated node.
    /// If you want a stack-allocated node, use [`AstBuilder::nota_link`] instead.
    ///
    /// ## Parameters
    /// * `span`: The [`Span`] covering this node
    /// * `url`: The raw url slice (inside `(…)`, untrimmed/uncooked).
    ...
    where
        A1: Into<Str<'a>>,
    {
        Box::new_in(self.nota_link(span, url, url_span, children), self.allocator)
    }

    /// Build a [`NotaImage`].
    ///
    /// If you want the built node to be allocated in the memory arena,
    /// use [`AstBuilder::alloc_nota_image`] instead.
    ///
    /// ## Parameters
    /// * `span`: The [`Span`] covering this node
    /// * `alt`: The raw alt-text slice (inside `[…]`, uncooked).
    ...
    /// Build a [`NotaImage`], and store it in the memory arena.
    ///
    /// Returns a [`Box`] containing the newly-allocated node.
    /// If you want a stack-allocated node, use [`AstBuilder::nota_image`] instead.
    ///
    /// ## Parameters
    /// * `span`: The [`Span`] covering this node
    /// * `alt`: The raw alt-text slice (inside `[…]`, uncooked).
    ...
    where
        ...
        A2: Into<Str<'a>>,
    {
        Box::new_in(self.nota_image(span, alt, alt_span, src, src_span), self.allocator)
    }
