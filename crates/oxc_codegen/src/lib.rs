...
pub struct Codegen<'a> {
    ...
    /// Nota source→generated offset log. `Some` once [`Codegen::with_nota_offset_log`] is
...
impl<'a> Codegen<'a> {
    ...
    fn add_source_mapping(&mut self, span: Span) {
        ...
        // Log `(source_start, source_end, generated_start)` for the Nota CodeMapping join.
    ...
    fn add_source_mapping_for_name(&mut self, span: Span, name: &str) {
        ...
        // An identifier maps 1:1 — `name` is printed right after this call at `code.len()`, and
