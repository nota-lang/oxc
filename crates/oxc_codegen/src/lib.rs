...
pub struct CodegenReturn {
    ...
    /// The Nota source→generated **offset log** (H1), populated iff
    /// [`Codegen::with_nota_offset_log`] was called. Each entry is
    /// `(source_start, source_end, generated_start)`: at every point codegen mapped an AST *leaf*
    /// with a non-empty source span, the node's source span and the byte offset in the generated
    /// output where its text began. The `oxc::nota` compile entry intersects this (by source range)
    /// with the reader's `NotaMappingMark`s to build per-leaf Volar `CodeMapping`s — see
    /// `oxc_parser`'s `nota_mapping` module and contract §4 H1.
    ///
...
pub struct Codegen<'a> {
    ...
    /// Nota source→generated offset log (H1). `Some` once [`Codegen::with_nota_offset_log`] is
    /// called: each source-mapped node appends `(source_start, source_end, generated_start)`.
    /// `None` ⇒ no logging (zero cost for the normal codegen / build path).
    /// See [`CodegenReturn::nota_offset_log`].
    #[cfg(feature = "sourcemap")]
    nota_offset_log: Option<Vec<(u32, u32, u32)>>,
}
...
impl<'a> Codegen<'a> {
    ...
    /// Enable the Nota source→generated **offset log** (H1).
    ///
    ...
    /// This is the generated-offset oracle the `oxc::nota` compile entry pairs with the reader's
    /// `NotaMappingMark`s to build Volar `CodeMapping`s. Requires [`CodegenOptions::source_map_path`]
    ...
    pub fn with_nota_offset_log(mut self) -> Self {
        self.nota_offset_log = Some(Vec::new());
        self
    }
    ...
    pub fn build(mut self, program: &Program<'a>) -> CodegenReturn {
        ...
        #[cfg(feature = "sourcemap")]
        let nota_offset_log = self.nota_offset_log.unwrap_or_default();
        CodegenReturn {
            code,
            #[cfg(feature = "sourcemap")]
            map,
...
impl<'a> Codegen<'a> {
    ...
    fn add_source_mapping(&mut self, span: Span) {
        ...
        // H1: log `(source_start, source_end, generated_start)` for the Nota CodeMapping join.
        // `code.len()` is the generated byte offset where this node's text begins (callers print
        // after this call).
        if let Some(log) = self.nota_offset_log.as_mut() {
            #[expect(clippy::cast_possible_truncation)]
            log.push((span.start, span.end, self.code.len() as u32));
        }
    }
    ...
    fn add_source_mapping_for_name(&mut self, span: Span, name: &str) {
        ...
        // H1: an identifier maps 1:1 — `name` is printed right after this call at `code.len()`, and
        // its source/generated lengths are equal (`span.len() == name.len()`).
        if let Some(log) = self.nota_offset_log.as_mut() {
            #[expect(clippy::cast_possible_truncation)]
            log.push((span.start, span.end, self.code.len() as u32));
        }
